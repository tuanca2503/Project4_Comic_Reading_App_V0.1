class ResponseErrorHelper {
  ...
  static final Map<String, String> _responseUserMessage = {
    ...
    "Code forgot password is not correct or expired": "Phiên đăng nhập đã hết hạn!",
    "Password is not correct": "Mật khẩu hiện tại không đúng!",
  };

  static String getErrorMessage(dynamic error) {
    String? errorMessage;
    if (error?.message?.response?.data is Map && error.message.response.data.containsKey('message')) {
      errorMessage = _responseUserMessage[error.message.response.data['message']];
    }
